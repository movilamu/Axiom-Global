...
export function computeRiskScore(
  ...
  const pausePenalty = Math.min(
    biometrics.keystroke.longPauses * T.PAUSE_PENALTY,
    T.PAUSE_PENALTY_CAP
  );
  const keystrokeRaw = dwellDev * 50 + flightDev * 50 + pausePenalty;
  const keystrokeScore = Math.min(keystrokeRaw, 100) * 0.4;
  if (dwellDev > 0.5) factors.push("High keystroke dwell deviation");
  if (flightDev > 0.5) factors.push("High keystroke flight deviation");
  ...
  if (biometrics.mouse.averageVelocity > 800) {
    ...
    biometrics.mouse.averageVelocity < 50 &&
