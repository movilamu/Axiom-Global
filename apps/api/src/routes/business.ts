...
const profileStore = new Map<string, { id: string; userId: string; businessName: string; category: string; monthlyLimit: number }>();
const expensesStore = new Map<string, { id: string; profileId: string; merchant: string; category: string; amount: number; currency: string; date: string; note?: string }[]>();
const cardsStore = new Map<string, { id: string; profileId: string; last4: string; expiryMonth: number; expiryYear: number; isActive: boolean; spendLimit?: number }[]>();
...
function getExpenses(profileId: string) {
  if (!expensesStore.has(profileId)) expensesStore.set(profileId, []);
  return expensesStore.get(profileId)!;
}

function getCards(profileId: string) {
  if (!cardsStore.has(profileId)) cardsStore.set(profileId, []);
...
router.post("/profile", (req, res) => {
  ...
  if (monthlyLimit != null) profile.monthlyLimit = Number(monthlyLimit);
  return res.json(profile);
});
...
router.patch("/cards/:id/toggle", (req, res) => {
  ...
  if (!card) return res.status(404).json({ error: "Card not found", code: "NOT_FOUND" });
