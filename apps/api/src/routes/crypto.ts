...
let pricesCache: { data: CoinPrice[]; ts: number } | null = null;
...
async function fetchPrices(): Promise<CoinPrice[]> {
  ...
  try {
    const res = await fetch(
      "https://api.coingecko.com/api/v3/coins/markets?vs_currency=usd&ids=bitcoin,ethereum,tether,solana,bnb,xrp,usdc,cardano,avalanche-2,dogecoin,chainlink,polkadot,polygon,litecoin&sparkline=true&price_change_percentage=24h"
    );
...
function getHoldings(userId: string) {
  if (!holdingsStore.has(userId)) holdingsStore.set(userId, []);
