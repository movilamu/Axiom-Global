...
function getGoals(userId) {
    if (!goalsStore.has(userId))
        goalsStore.set(userId, []);
...
router.post("/", (req, res) => {
    try {
        ...
        const { name, targetAmount, currency, emoji, deadline, autoSaveRule } = parsed.data;
        const goals = getGoals(userId);
...
router.patch("/:id", (req, res) => {
    ...
    if (updates.name != null)
        goal.name = String(updates.name);
    if (updates.targetAmount != null)
        goal.targetAmount = Number(updates.targetAmount);
    if (updates.emoji != null)
        goal.emoji = String(updates.emoji);
    if (updates.deadline != null)
        goal.deadline = String(updates.deadline);
    if (updates.autoSaveRule != null)
        goal.autoSaveRule = updates.autoSaveRule;
    if (updates.currentAmount != null)
        goal.currentAmount = Number(updates.currentAmount);
    if (updates.isCompleted != null)
        goal.isCompleted = Boolean(updates.isCompleted);
